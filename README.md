...
本ツールの使用にはアクティベーションキーが必要です。

**手順はこれだけです：**

1. プロジェクトフォルダ内の `activate.key` をテキストエディタで開く
2. `PASTE-YOUR-KEY-HERE` という文字を、受け取ったキーに書き換えて保存する
...
起動時に `✅ 認証OK（有効期限: YYYY-MM-DD）` と表示されれば設定完了です。

> キーは他の方と共有しないでください。有効期限が切れると自動的に停止します。
